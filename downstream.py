...
def main(args):
    ...
    if args.pretrain_task is PretrainTask.none:
        ...
        #model = PatchPredictionNetwork(transformer=model, patch_size=16, dim=1024,mask_prob=0.15, replace_prob=0.50).cuda()
...
# validation function.
def validate(loader, model, criterion, epoch, args):
    total_loss = 0
    total_accuracy = 0
    total = 0
    model.eval()
